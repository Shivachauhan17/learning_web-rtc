...
socket.on('created',function(channel){
    ...
    console.log("peer is the initiator")
...
socket.on('full',function(channel){
    console.log('channel'+channel+'is too crowded!Cannot allow you to enter')
    // div.insertAdjacentHTML('beforeend','<p>Time:'+(performance.now()/1000).toFixed()+'-->channel'+channel+'is too crowded! Cannot allow you to enter')
})
...
socket.on('Bye',function(){
    // console.log('Got "Bye" from other peer going to discoonect..')
    // div.insertAdjacentHTML( 'beforeend', '<p>Time: ' +
    // (performance.now() / 1000).toFixed(3) +
